...
class KalmanNetNN(torch.nn.Module):
    ...
    def NNBuild(self, h, m, n, prior_Q, prior_Sigma, prior_S, t_fa, v_fa, args):
        ...
        self.InitKGainNet(prior_Q, prior_Sigma, prior_S, args)
    ...
    def forward(self, y, t):
        ...
        F = torch.tensor([[1, -(self.t_fa[t + 1001] - self.t_fa[t+1000]),
                                  ...
                                  [0, 0, 1, 0],
                                  [0, 0, 0, 1]]).float()
        batched_F = F.to(self.m1x_posterior.device).view(1, F.shape[0], F.shape[1]).expand(
            self.m1x_posterior.shape[0], -1, -1)
        self.m1x_prior = torch.bmm(batched_F,
                                   self.m1x_posterior)  #########################################调整########################
        ...
        # Innovation
        dy = y - self.m1y  # [batch_size, n, 1]  y = torch.unsqueeze(y_training_batch[:, :, t],
    ...
    def init_hidden_KNet(self):
        ...
        self.h_S = self.prior_S.flatten().reshape(1, 1, -1).repeat(self.seq_len_input, self.batch_size,
                                                                   1)  # batch size expansion reshape = 1,m**2
