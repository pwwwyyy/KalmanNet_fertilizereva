...
class Pipeline_EKF:
    ...
    def setssModel(self, m, n, T, m1x_0, h, T_test):
        self.m = m
        self.n = n
        self.T = T
        self.h = h
    ...
    def NNTrain(self, cv_input, cv_target, train_input, train_target, path_results, \
        ...
        for ti in range(0, self.N_steps):
            ...
            y_training_batch = torch.zeros([self.N_B, self.n, self.T]).to(self.device)
            train_target_batch = torch.zeros([self.N_B, self.m, self.T]).to(self.device)
            x_out_training_batch = torch.zeros([self.N_B, self.m, self.T]).to(self.device)
            ...
            # Init Sequence
            if (randomInit):
                train_init_batch = torch.empty([self.N_B, self.m, 1]).to(self.device)
                ii = 0
                for index in n_e:
                    train_init_batch[ii, :, 0] = torch.squeeze(train_init[index])
                    ii += 1
                self.model.InitSequence(train_init_batch, self.T)
            else:
                self.model.InitSequence( \
                    self.m1x_0.reshape(1, self.m, 1).repeat(self.N_B, 1, 1), self.T)


            # Forward Computation###############################################################################################################
            for t in range(0, self.T):
                x_out_training_batch[:, :, t] = torch.squeeze(self.model(torch.unsqueeze(y_training_batch[:, :, t], 2), t))
            # Compute Training Loss
            MSE_trainbatch_linear_LOSS = 0
            if (self.args.CompositionLoss):
                y_hat = torch.zeros([self.N_B, self.n, self.T])
                for t in range(self.T):
                    y_hat[:, :, t] = torch.squeeze(self.h(torch.unsqueeze(x_out_training_batch[:, :, t])))
            ...
            with torch.no_grad():
                ...
                x_out_cv_batch = torch.empty([self.N_CV, self.m, self.T_test]).to(self.device)

                # Init Sequence
                if (randomInit):
                    if (cv_init == None):
                        self.model.InitSequence( \
                            self.m1x_0.reshape(1, self.m, 1).repeat(self.N_CV, 1, 1), self.T_test)
                    else:
                        self.model.InitSequence(cv_init, self.T_test)
                else:
                    self.model.InitSequence( \
                        self.m1x_0.reshape(1, self.m, 1).repeat(self.N_CV, 1, 1), self.T_test)

                ##########################################################################################################################
                for t in range(0, self.T_test):
                    x_out_cv_batch[:, :, t] = torch.squeeze(self.model(torch.unsqueeze(cv_input[:, :, t], 2), t))
                ...
                if (self.MSE_cv_dB_epoch[ti] < self.MSE_cv_dB_opt):
                    ...
                    torch.save(self.model, path_results + 'best-model400-100-13.pt')
    ...
    def NNTest(self, test_input, test_target, path_results, MaskOnState=False, \
        ...
        if load_model:
            ...
            self.model = torch.load(path_results + 'best-model400-100-13.pt', map_location=self.device)

        self.N_T = test_input.shape[0]
        self.T_test = test_input.size()[-1]
        self.MSE_test_linear_arr = torch.zeros([self.N_T])
        x_out_test = torch.zeros([self.N_T, self.m, self.T_test]).to(self.device)
        KG_out_test = torch.zeros([self.m, self.n, self.T_test]).to(self.device)
        if MaskOnState:
            mask = torch.tensor([True, False, False, False])
            if self.m == 2:
                mask = torch.tensor([True, False])
        ...
        if (randomInit):
            self.model.InitSequence(test_init, self.T_test)
        else:
            self.model.InitSequence(self.m1x_0.reshape(1, self.m, 1).repeat(self.N_T, 1, 1), self.T_test)

        for t in range(0, self.T_test):
            x_out_test[:, :, t] = torch.squeeze(self.model(torch.unsqueeze(test_input[:, :, t], 2), t))
            KG_out_test[:, :, t] = self.model.KGain[0]
            # print("R", self.model.h_S, "Q", self.model.h_Q)
        end = time.time()
        t = end - start
        ...
        return [self.MSE_test_linear_arr, self.MSE_test_linear_avg, self.MSE_test_dB_avg, x_out_test, t, KG_out_test]
