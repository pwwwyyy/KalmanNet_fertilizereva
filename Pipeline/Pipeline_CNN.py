...
class Pipeline_CNN:
    ...
    def setssModel(self, m, n, T, h, T_test, m2x_1000, H_onlyPos, Q_gen, R_onlyPos):
        self.m = m
        self.n = n
        self.T = T
        self.h = h
        self.T_test = T_test
        self.m2x_1000 = m2x_1000
        self.H_onlyPos = H_onlyPos
        self.Q_gen = Q_gen
        self.R_onlyPos =R_onlyPos
    ...
    def NNTrain(self, cv_input, cv_target, train_input, train_target, path_results, \
        ...
        for ti in range(0, self.N_steps):
            ...
            # Init sliding window
            self.model.InitSW(y_training_batch, train_target_batch, self.m2x_1000, self.H_onlyPos, self.Q_gen, self.R_onlyPos)

            # Init Sequence
            self.model.InitSequence(y_training_batch, self.T)

            # Forward Computation
            for t in range(0, self.model.sw):
                x_out_training_batch[:, :, t] = self.model.x_posterior_sw0[:, :, t]

            for t in range(self.model.sw, self.T):
                x_out_training_batch[:, :, t] = self.model(y_training_batch[:, :, t], t)
                # print(x_out_training_batch[:, :, t])
                # print(self.model.KGain)
            ...
            with torch.no_grad():
                ...
                # Init sliding window
                self.model.InitSW(cv_input, cv_target, self.m2x_1000, self.H_onlyPos, self.Q_gen, self.R_onlyPos)
                # Init Sequence
                self.model.InitSequence(cv_input, self.T_test)

                for t in range(0, self.model.sw):
                    x_out_cv_batch[:, :, t] = self.model.x_posterior_sw0[:, :, t]

                for t in range(self.model.sw, self.T_test):
                    x_out_cv_batch[:, :, t] = self.model(cv_input[:, :, t], t)
    ...
    def NNTest(self, test_input, test_target, path_results, MaskOnState=False, \
        ...
        self.model.InitSW(test_input, test_target, self.m2x_1000, self.H_onlyPos, self.Q_gen, self.R_onlyPos)  # , self.m2x_1000, self.H_onlyPos, self.Q_gen, self.R_onlyPos
        self.model.InitSequence(test_input, self.T_test)

        for t in range(0, self.model.sw):  # ##########################################################################
            x_out_test[:, :, t] = self.model.x_posterior_sw0[:, :, t]
